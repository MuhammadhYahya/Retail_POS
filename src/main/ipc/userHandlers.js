...
export function registerUserHandlers() {
  ...
  ipcMain.handle('user:updatePermissions', async (event, payload = {}) => {
    try {
      const token = extractToken(payload);
      const session = validateSession(token);
      if (!session.success) return session;
      ...
      const user = userService.updatePermissions(userId, payload.permissions);
      writeAuditLog(`user_permissions_update:${user.username}`, session.user.id);
      return { success: true, data: user };
    } catch (err) {
      return { success: false, error: err.message };
    }
  ...
  ipcMain.handle('user:resetPin', async (event, payload = {}) => {
    try {
      ...
      if (target.role !== 'cashier' && target.role !== 'manager') {
        return { success: false, error: 'Only cashier or manager PINs can be reset from Staff Management.' };
      }
